...
function qBranchPicker() {
	...
	var checkInventorySpace = function() {
		if (!goingBackToPile) {
			if (g.getFreeCharInvCellsCount() < 5)
				invFull = true;
			else
				invFull = false;
		}
	...
	var findClosestEmptyStockPile = function() {
		...
		for (var i = 0; i < mapObjects.length; ++i) {
			...
			else if (isBranchStockpile(mapObjects[i].name)) {
	...
	var storeStockpile = function() {
		...
		if (g.getStockpileInfo() != null) {
			...
			print('Putting branches into stockpile...');
	...
	var checkInventoryForBranches = function()
	{
		var items = g.getInvItems();
		for (var i = 0; i < items.length; i++) {
			if (items[i] == 'branch') {
